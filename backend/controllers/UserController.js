...
export const subscribe = async (req, res) => {
  try {
    ...
    const userToSubscribe = await UserModel.findByIdAndUpdate(
      userToSubscribeId,
      {
        $push: {
    ...
    const folower = await UserModel.findByIdAndUpdate(
      followerId,
      {
        $push: {
...
export const savePost = async (req, res) => {
  try {
    ...
    const user = await UserModel.findByIdAndUpdate(_id, {
      $push: { markedPosts: req.params.id },
    });
    return res.status(200).json(user);
  } catch (err) {
    console.log(err);
...
export const removeFromsavedPosts = async (req, res) => {
  try {
    const { _id } = req.user;
    const user = await UserModel.findByIdAndUpdate(_id, {
      $pull: { markedPosts: req.params.id },
    });
    return res.status(200).json(user);
  } catch (err) {
    console.log(err);
